...
int main(int argc, char ** argv)
{
	...
	if (ccnu_publish(&index) < 0) {
		printf("could not publish index segment!\n");
		exit(EXIT_FAILURE);
	}
	printf("published index segment\n");
	...
	while (fileLen > 0) {
		...
		sprintf(str, "%s/%d", con_name, segment);
		printf("Creating segment %d (name = %s), size = %d\n", segment, str, n);
		struct content_obj seg;		
		seg.name = content_name_create(str);
		seg.size = n;
		seg.data = malloc(n);
		seg.timestamp = ts.tv_sec;
		memcpy(seg.data, buffer, n);
		if (ccnu_publish(&seg) < 0) {
			printf("could not publish segment %d!\n", segment);
			exit(EXIT_FAILURE);
		}
