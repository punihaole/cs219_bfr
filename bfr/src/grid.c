...
int grid_init(unsigned int levels, unsigned int width, unsigned int height)
{
    ...
    log_print(g_log, "grid_init: creating %d levels - %dx%d", levels, width, height);
                  ...
                  double x, double y, double * distance)
{
    ...
    if (!distance)
        return -1;
    if (x > g_grid.x_bounds[level-1][g_grid.divisions[level-1]-1] || x < 0)
        return -1;
    if (y > g_grid.y_bounds[level-1][g_grid.divisions[level-1]-1] || y < 0)
        return -1;
