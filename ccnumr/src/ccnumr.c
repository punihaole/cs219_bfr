...
inline void ccnumr_did2sockpath(uint32_t daemonId, char * str, int size)
{
    snprintf(str, 256, "/tmp/ccnumr_%u.sock", daemonId);
}

void ccnumr_msg_delete(struct ccnumr_msg * msg)
{
...
int ccnumr_sendMsg(struct ccnumr_msg * msg)
{
    if (!msg) {
        fprintf(stderr, "ccnumr_sendMsg: msg NULL -- IGNORING.");
        return -1;
    }

    if (!msg->payload.data) {
        fprintf(stderr, "ccnumr_sendMsg: payload NULL -- IGNORING.");
        return -1;
    }
    ...
    char sock_path[256];
    ccnumr_did2sockpath(IP4_to_nodeId(), sock_path, 256);
...
int ccnumr_sendLoc(double x, double y)
{
    int s, len;
    struct sockaddr_un daemon;
...
int ccnumr_sendQry(struct content_name * name,
                  ...
                  double * last_hop_distance, int * need_fwd)
{
    if (!name || !dest_level || !dest_clusterId || !need_fwd) {
        fprintf(stderr, "ccnumr_sendQry: invalid parameter(s) -- IGNORING.");
        return -1;
    }
...
int ccnumr_sendWhere(struct content_name * name,
                    unsigned * orig_level, unsigned * orig_clusterId,
                    unsigned * dest_level, unsigned * dest_clusterId,
                    double * distance)
{
    if (!name || !dest_level || !dest_clusterId || !distance) {
        fprintf(stderr, "ccnumr_sendWhere: invalid parameter(s) -- IGNORING.");
        return -1;
    }
...
int ccnumr_sendDistance(struct content_name * name, int hops)
{
    if (!name) {
