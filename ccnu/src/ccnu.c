...
static int verify_content(struct content_obj * content)
{
    ...
    if (content->size > ccnu_max_payload_size(content->name)) {
...
{
    snprintf(str, 256, "/tmp/ccnud_%u.sock", daemonId);
}

inline int ccnu_max_payload_size(struct content_name * name)
{
    return CCNU_MAX_PACKET_SIZE - MIN_DATA_PKT_SIZE - name->len;
}
...
int ccnu_publishSeq(struct content_obj * index_obj, struct linked_list * chunks)
{
    if (verify_content(index_obj) != 0) {
        fprintf(stderr, "ccnu_publish: found a problem with content -- IGNORING");
        return -1;
    }
...
{
    if (!bloom_ptr) {
        syslog(LOG_WARNING, "ccnu_cs_summary: invalid bloom_ptr! -- IGNORING");
        return -1;
    }
